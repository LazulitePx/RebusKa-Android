...
object PublicacionRepository {
    ...
    fun getPublicacionById(id: Int): Publicacion? = publicaciones.find { it.id == id }
    fun getPublicacionesByNegocio(idNegocio: Int): List<Publicacion> = publicaciones.filter { it.idNegocio == idNegocio }
    fun getPublicacionesByTipo(tipo: TipoPublicacion): List<Publicacion> = publicaciones.filter { it.tipo == tipo }

    fun actualizarPublicacion(id: Int, titulo: String, descripcion: String, precio: Long) {
        val i = publicaciones.indexOfFirst { it.id == id }
        if (i != -1) publicaciones[i] = publicaciones[i].copy(titulo = titulo, descripcion = descripcion, precio = precio)
    }
    fun eliminarPublicacion(id: Int) { publicaciones.removeIf { it.id == id } }
}